...
var TwitOverlay = {
	...
	openPrefs:
		function () {
			var dialog = this.windowService.getMostRecentWindow("TwitKit:Preferences");
			if (dialog) {
				dialog.focus();
				return;
			}
	...
	openAbout:
		function () {
			var dialog = this.windowService.getMostRecentWindow("TwitKit:About");
			if ( dialog ) {
				dialog.focus();
				return;
			}
			openDialog(
				"chrome://twitkit/content/about.xul",
				"_blank",
				"chrome,titlebar,centerscreen," +
				( ( this.getPref("browser.preferences.instantApply", false ) ) ? "dialog=no" : "modal" )
			);
